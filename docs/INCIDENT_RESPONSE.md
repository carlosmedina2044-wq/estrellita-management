...
- 2026-09-17 — Engagement plan Phases 1–4 (agent): dedupe fails closed with every catalog task tagged; window-to-room map; house sheet; year screen with on-device `checkIns`; care history; house line; get-ahead card; year intro and year wrapped; evening nudge (off by default, ids 3000–3006); picture share cards; widget target added to the Xcode project (`scripts/ios/add-widget-target.rb`) and the house on the widget via layer files copied into the App Group; weather alerts via a plaintext watch list in standard defaults plus a `BGAppRefreshTask` (`WeatherRefresh.swift`, `UIBackgroundModes` fetch); vault refuses writes from a module instance that has read nothing. Device checks still due: App Group in the portal, widget on device, `_simulateLaunchForTaskWithIdentifier:` for the weather task, evening nudge firing, share sheet with a file.
