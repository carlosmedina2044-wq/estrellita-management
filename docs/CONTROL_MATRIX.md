...
| Vault key is biometric/passcode-bound; decrypt only after ACL success | MASVS-AUTH-2 | `CuidalaDeviceKeyPlugin` SecAccessControl; `face-lock.tsx` → `unlockHousehold`; lock clears CryptoKey + plaintext | Documented in `RESIDUAL_RISKS.md`; unit tests for session lock/unlock | VERIFIED (JS session), NOT VERIFIED (device ACL) |
