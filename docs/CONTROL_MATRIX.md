...
| Key is random, device-scoped, never derived from a PIN | MASVS-CRYPTO-1 | `src/lib/native/device-key.ts` + `CuidalaDeviceKeyPlugin` (Keychain add-or-update, `kSecAttrAccessibleAfterFirstUnlock`; still reads leftover `cap_sec` items) | Unit test rejects non-32-byte keys; missing-vs-other Keychain errors in `device-key.test.ts` | VERIFIED (key length + missing-item), NOT VERIFIED (Keychain attributes on device) |
| Vault is read before any key mint; never overwrite a vault with a fresh key | S1 | `hydrateHousehold` loads vault first and never mints. `persist` reuses a Keychain key that can open the vault, or a leftover Keychain item when no vault exists; if the blob is unreadable it quarantines then mints. Keychain errors while a vault exists still refuse to mint. | `vault.test.ts` key-mismatch / unavailable / first-launch mint / S1d–S1g | VERIFIED |
