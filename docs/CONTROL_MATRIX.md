...
| Vault is read before any key mint; never overwrite a vault with a fresh key | S1 | `hydrateHousehold` loads vault first and never mints. `persist` reuses a Keychain key that can open the vault; if the blob is unreadable (empty Keychain or wrong key) it quarantines then mints. Keychain errors while a vault exists still refuse to mint. | `vault.test.ts` key-mismatch / unavailable / first-launch mint / S1d–S1f | VERIFIED |
