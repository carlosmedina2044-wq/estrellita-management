...
| Privacy manifest matches data flows | Apple privacy manifest | `PrivacyInfo.xcprivacy` — tracking false; collected types empty (WeatherKit is Apple-collected); UserDefaults CA92.1; File Timestamp C617.1 (`@capacitor/filesystem`) | Reviewed against this matrix and `privacy/page.tsx` | VERIFIED |
| Face ID and location usage strings present in shipped Info.plist | App Store 5.1.1 | `ios/App/App/Info.plist` — location string states ZIP/location is sent to Apple for geocoding/WeatherKit; Cuidala has no server | File inspection + privacy copy in catalogs | VERIFIED |
